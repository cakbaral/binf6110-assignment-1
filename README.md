...
## Proposed Methods
ONT R10 raw reads of *Salmonella enterica* were obtained from the SRR32410565 isolate on the National Centre for Biotechnology Information (NCBI) database (NCBI, 2023).  Its designated reference genome was obtained from the ASM694v2 genome assembly of *Salmonella enterica subsp. enterica serovar Typhimurium str. LT2*  (NCBI, 2016).  The reads then assembled using de novo assembly with Flye v2.9.6-b1802 with 32 threads, a more recent model closer to the one that has been used prior for serotyping (Lamas et al., 2023).  The assembly was repeated three times to check for major discrepancies in the sequence statistics (ex. sequence length, number of contigs, etc.).  Sequence statistics were obtained using the "seqkit" package from Bioconda (Shen et al., 2016).  After confirming no major discrepancies, the resulting consensus Flye assembly then underwent polishing steps using the software Medaka v2.2.0 with 16 threads and the *r103_sup_g507* model for R10 flowcells, and sequence statistics were re-obtained from the seqkit package (Oxford Nanopore Technologies Ltd., 2018, Anaconda, n.d., Shen et al., 2016).  Medaka has often displayed more accuracy with Nanopore samples, correcting more errors, and it has also been used alongside Flye for polishing assemblies for serotyping research (Luan et al., 2024, Lamas et al., 2023).  The consensus genomes were then mapped and visualized using the Proksee web software (Grant et al., 2023).  For quality control (QC) steps, QUAST v5.3.0 was used to evaluate the quality of the Flye consensus genome and the polished Medaka consensus genome against the *Salmonella enterica* reference genome from NCBI (Mikheenko et al., 2024, NCBI, 2023) to compare differences.  QUAST has been a valuable quality control tool for comparing bacterial genome assemblies against their reference genomes (Gurevich et al., 2013).  Once the consensus genomes were assessed for quality, Minimap2 v2.30-r1287 was used with 32 threads to align each one with the reference genome (Li, 2018, Li et al., 2025, NCBI, 2023).  For variant calling analysis, Clair3 v1.1.2 r1041_e82_400bps_sup_v500, a deep learning model, was used in a Singularity/Apptainer container with v1.4.5 (Zheng et al., 2025, Kurtzer et al., 2017).  Finally, for visualization of the alignment in the Integrative Genomics Viewer (IGV) v2.19.7, SAMtools v1.2.3 was used to convert, sort, and index into BAM format (Robinson et al., 2011, IGV, n.d., Li et al., 2009, SAMtools, n.d.).

## Results
When comparing the plasmid consensus maps of Salmonella enterica, both the Flye assembly and its polished Medaka assembly, each resulted in a total sequence length of about 5.09 million base pairs, across 9 contigs.  Compared to the reference genome, which had a length of about 4.96 million base pairs but only contained 2 contigs, this meant that there were more fragments while joining the ONT-R10 raw reads.  Medaka polishing did, however, affect the locations of the contigs, as there were no adjacencies that were retained from the Flye assembly.  For quality control from the Quality Assessment Tool for Genome Assemblies (QUAST), there were no major differences in the reported statistics.  According to the genome fraction, about 95% of the reference genome was covered by the assembly.  However, a total of 20 misassemblies were also found, all of which were reported as relocation errors.  For regions that were unaligned, QUAST reported 1 fully unaligned contig and 7 partially unaligned contigs.  Lastly, the N50 was calculated to approximately at least 1.25 million base pairs.  According to the consensus maps, exactly two contigs were of this length or greater.
